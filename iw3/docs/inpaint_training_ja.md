...
1. 深度マップを使って元画像の深度マップを**左**ビュー用にワープ
2. 左ビュー用にワープした深度マップを使って空のテンソルを**右**ビュー用にワープ
3. 元画像上にforward warpingによる欠損領域が生成される
4. 生成された欠損領域をクロージング処理で補完

左ビュー用にワープした画像を同じパラメーターで右ビュー用にワープさせると元画像の位置に戻るという性質を使って、元画像に対する欠損領域を生成します。元画像にはワープによる歪みや欠損がないため、正解データ（Ground Truth）として利用可能です。
