...
CYCLES=1
NUM_SAMPLES=25000
FORCE_SAVE_MODEL="" # use "--update-criterion all" if needed. validation does not support all photo noise so it may no longer update the best models.
LOSS="--loss lbp5m" # lbp5 for 4x, lbp5m for unif4x (includes downscaled 2x training)
DA_OPTIONS="--deblur 0.05 --da-scale-p 0.75 --da-unsharpmask-p 0.2 --da-grayscale-p 0.01"
