...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=640,
        help="maximum image size. If the face image is larger than max-size, resize it to max-size.",
    )
    ...
    if args.cascade_file is not None:
        ...
        classifier = cv2.CascadeClassifier(path.join(cv2.data.haarcascades, "haarcascade_frontalface_alt2.xml"))

    for filename in tqdm(os.listdir(args.input_dir), ncols=80):
        if path.splitext(filename)[-1].lower() not in {".png", ".jpg", ".jpeg", ".webp"}:
            continue
        ...
        for x, y, w, h in faces:
            for margin in args.margin:
                ...
                if max_size > args.max_size:
                    ...
                    face = cv2.resize(
                        face, (int(face.shape[1] * scale), int(face.shape[0] * scale)), interpolation=cv2.INTER_AREA
                    )
