...
def dct_loss(input, target, loss_function=F.l1_loss):
    return loss_function(dct2(input), dct2(target))


def window_dct_loss(input, target, window_size=8, loss_function=F.l1_loss):
    ...
    return dct_loss(input, target, loss_function=loss_function)


def overlap_window_dct_loss(input, target, window_size=8, loss_function=F.l1_loss):
    ...
    dct1 = window_dct_loss(input, target, window_size=window_size, loss_function=loss_function)
    dct2 = window_dct_loss(input2, target2, window_size=window_size, loss_function=loss_function)
    return dct1 * 0.5 + dct2 * 0.5


class DCTLoss(nn.Module):
    # BCHW
    def __init__(self, window_size=None, overlap=False, loss_function="l1"):
        super().__init__()
    ...
    def forward(self, input, target):
        ...
        if self.window_size is not None:
            if self.overlap:
                return overlap_window_dct_loss(input, target, window_size=self.window_size, loss_function=self.loss_function)
            else:
                return window_dct_loss(input, target, window_size=self.window_size, loss_function=self.loss_function)
        else:
            return dct_loss(input, target, loss_function=self.loss_function)
