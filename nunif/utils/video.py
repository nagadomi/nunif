...
def hdr2sdr(
        ...
        hlg_exposure=1.2, hlg_white_point=0.8, hlg_saturation_gain=0.9,
        device="cpu"
):
    # Based on the code generated by Gemini
    ...
    # 2. Inverse EOTF: Convert Non-linear to Linear
    if color_trc == COLOR_TRC_SMPTE2084:
        # --- PQ (HDR10) Logic ---
        m1, m2 = 2610 / 16384, 2523 / 4096 * 128
        c1, c2, c3 = 3424 / 4096, 2413 / 4096 * 32, 2392 / 4096 * 32
        x_pow = torch.pow(x, 1.0 / m2)
        linear_2020 = torch.pow(torch.clamp(x_pow - c1, min=0) / (c2 - c3 * x_pow), 1.0 / m1)
        current_exposure = pq_exposure
        current_white = pq_white_point
    else:
        # --- HLG (ARIB STD-B67) Logic ---
        a, b, c = 0.17883277, 0.28466892, 0.55991073
        # Inverse EOTF
        linear_2020 = torch.where(
            x <= 0.5,
            torch.pow(x, 2.0) / 3.0,
            (torch.exp((x - c) / a) + b) / 12.0
        )
        # Apply System Gamma (OOTF) for HLG -> SDR
        # Typically 1.2 for HLG display to linear reference.
        # linear_2020 = torch.pow(linear_2020, 1.0)

        # exposure: HLG is already SDR-compatible. 1.0 means no boost.
        current_exposure = hlg_exposure
        current_white = hlg_white_point

    # 3. Tone Mapping (Hable Algorithm)
    x_lin = linear_2020 * current_exposure

    def hable_map(v, E=0.02):
        A, B, C, D, E, F = 0.15, 0.50, 0.10, 0.20, E, 0.30
        return ((v * (A * v + C * B) + D * E) / (v * (A * v + B) + D * F)) - E / F

    if color_trc == COLOR_TRC_ARIB_STD_B67:
        sdr_linear_2020 = (hable_map(x_lin, E=0.01) /
                           hable_map(torch.tensor(current_white, dtype=x.dtype, device=x.device), E=0.01))
        if hlg_saturation_gain < 1.0:
            luma = (sdr_linear_2020[0] * 0.2126 +
                    sdr_linear_2020[1] * 0.7152 +
                    sdr_linear_2020[2] * 0.0722)
            sdr_linear_2020 = (sdr_linear_2020 * hlg_saturation_gain) + (luma * (1.0 - hlg_saturation_gain))
    else:
        sdr_linear_2020 = hable_map(x_lin) / hable_map(torch.tensor(current_white, dtype=x.dtype, device=x.device))

    # 4. Color Matrix: BT.2020 -> BT.709/601
    if output_colorspace == "bt709":
        ...
        matrix = torch.tensor([
            [1.5540, -0.5143, -0.0397],
            [-0.1017, 1.1147, -0.0130],
            [-0.0163, -0.0886, 1.1049]
        ], dtype=x.dtype, device=x.device)

    # Flatten (C, H, W) -> (C, H*W) for matrix multiplication.
    c, h, w = sdr_linear_2020.shape
    sdr_linear_sd = torch.mm(matrix, sdr_linear_2020.reshape(c, -1)).reshape(c, h, w)
    sdr_linear_sd = torch.clamp(sdr_linear_sd, 0, 1)

    # 5. SDR OETF (BT.709/601 Gamma)
    sdr_gamma = torch.where(
        sdr_linear_sd < 0.018,
        sdr_linear_sd * 4.5,
        1.099 * torch.pow(sdr_linear_sd, 0.45) - 0.099
    )
